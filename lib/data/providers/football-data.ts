...
const FD_TEAM_MAP: Record<number, number> = {
  759: 1001,  // Argentina  (fd=759  -> canonical=1001)
  773: 1002,  // France     (fd=773  -> canonical=1002)
  764: 1003,  // Brazil     (fd=764  -> canonical=1003)
  66:  1004,  // England    (fd=66   -> canonical=1004)
}
...
export class FootballDataProvider implements DataProvider {
  ...
  async fetchFixtures(leagueId: number, season: number): Promise<Fixture[]> {
    const data = await this.fetch<FdMatchesResponse>('matches', {
      competitions: String(leagueId),
      season: String(season),
    })

    console.log(
      `[football-data] fetchFixtures: ${data.matches.length} matches for competition ${leagueId} season ${season}`
    )
